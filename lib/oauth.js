...
OAuth.prototype.concatParams = function (oa, sep, wrap) {
  ...
  const params = Object.keys(oa).filter(function (i) {
...
OAuth.prototype.onRequest = function (_oauth) {
  ...
  const uri = self.request.uri || {}
  const method = self.request.method || ''
  const headers = caseless(self.request.headers)
  const body = self.request.body || ''
  const qsLib = self.request.qsLib || qs
  ...
  const oa = self.buildParams(_oauth, uri, method, query, form, qsLib)
  ...
  switch (transport) {
    ...
    case 'query': {
      const href = self.request.uri.href += (query ? '&' : '?') + self.concatParams(oa, '&')
      self.request.uri = parseUrl(href)
      self.request.path = self.request.uri.pathname + self.request.uri.search
      break
    }
